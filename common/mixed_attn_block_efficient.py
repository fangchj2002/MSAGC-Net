...
from nnunet.network_architecture.ZGQ.GRL_ImageRestoration.models.common.mixed_attn_block import (
    AnchorProjection,
    CAB,
    CPB_MLP,
    QKVProjection,
)
from nnunet.network_architecture.ZGQ.GRL_ImageRestoration.models.common.ops import (
    window_partition,
    window_reverse,
)

from nnunet.network_architecture.ZGQ.GRL_ImageRestoration.models.common.swin_v1_block import Mlp
...
class AffineTransform(nn.Module):
    ...
    def __init__(self, num_heads):
        ...
        logit_scale = torch.log(10 * torch.ones((num_heads+1, 1, 1)))
        self.logit_scale = nn.Parameter(logit_scale, requires_grad=True)

        # mlp to generate continuous relative position bias
        self.cpb_mlp = CPB_MLP(2, num_heads+1)

    def forward(self, attn, relative_coords_table, relative_position_index, mask):
        B_, H, N1, N2 = attn.shape
        # attn1 = torch.clamp(self.logit_scale, max=math.log(1.0 / 0.01)).exp()
        # logit scale
        # attn2 = torch.clamp(self.logit_scale, max=math.log(1.0 / 0.01)).exp()
        # attn = attn *attn2
...
class WindowAttention(Attention):
    ...
    def forward(self, qkv, x_size, table, index, mask):
        ...
        qkv = qkv.reshape(B_, N, 4, self.num_heads, -1).permute(2, 0, 3, 1, 4)
...
class AnchorStripeAttention(Attention):
    ...
    def forward(
        ...
        anchor = anchor.view(-1, prod(anchor_stripe_size), C // 12)

        B_, N1, _ = qkv.shape
        N2 = anchor.shape[1]
        if C > 768:
            qkv = qkv.reshape(B_, N1, 8, self.num_heads + 1, -1).permute(2, 0, 3, 1, 4)
        else:
            qkv = qkv.reshape(B_, N1, 12, self.num_heads + 1, -1).permute(2, 0, 3, 1, 4)
        #qkv = qkv.reshape(B_, N1, 12, self.num_heads+1, -1).permute(2, 0, 3, 1, 4)
        q, k, v = qkv[0], qkv[1], qkv[2]
        #anchor = anchor.reshape(B_, N2, self.num_heads, -1).permute(0, 2, 1, 3)
        anchor = anchor.reshape(B_, N2, self.num_heads+1, -1).permute(0, 2, 1, 3)
        ...
        # merge windows
        if C > 768:
            x = x.view(B_, *stripe_size, C // 8)
        else:
            x = x.view(B_, *stripe_size, C // 12)
...
class MixedAttention(nn.Module):
    ...
    def __init__(
        ...
        #self.proj_z = nn.Linear(dim*3//4, dim*3//4)

        self.proj_drop = nn.Dropout(proj_drop)
        if dim== 256:
            self.conv_z = nn.Conv2d(dim // 4, dim, kernel_size=1, stride=1)
        else:
            self.conv_z = nn.Conv2d(dim *3 // 8, dim, kernel_size=1, stride=1)
        #self.conv_z = nn.Conv2d(dim//4,dim,kernel_size=1,stride=1)
...
class EfficientMixAttnTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        #self.norm_z = norm_layer(dim*3//4)
    ...
    def forward(self, x, x_size, all_table_index_mask):
        ...
        if self.args.local_connection:
            ...
            #         x
            #         + self.res_scale
            #         * self.drop_path(self.norm_z(self.attn(x, x_size, table_index_mask)))
            #         + self.conv(x, x_size)
            # )
