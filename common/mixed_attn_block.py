...
class QKVProjection(nn.Module):
    def __init__(self, dim, qkv_bias, proj_type, args):
        ...
        # if proj_type == "linear":
        #     self.body = nn.Linear(512, 1536, bias=qkv_bias)
        else:
...
class AnchorLinear(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, down_factor, pooling_mode, bias):
        ...
        self.reduction = nn.Linear(in_channels, out_channels-64, bias=bias)
    ...
    def forward(self, x, x_size):
        ...
        x_r = self.reduction(x)
        x = blc_to_bhwc(x_r, [s // self.down_factor for s in x_size])
