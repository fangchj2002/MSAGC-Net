...
from gcn_lib import Grapher, act_layer, AttentionGrapher
...
class VIGUNet(torch.nn.Module):
    def __init__(self,num_classes=2, drop_path_rate=0.0, drop_rate=0.0, num_knn=9 ):
    ...
    def forward(self, x):
        ...
        feathers = feather[::-1]
        feathers.append(x)
        feathers = self.SpatialAwareTrans(feathers)
        x = feathers[3]
...
class MSF_VIGUNet(torch.nn.Module):
